...
public class StudentFeeForm extends JFrame implements ActionListener{
	...
	contentPane.add(t3);
        ...
        try{
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select * from user where ID = '"+c1.getSelectedItem()+"'");
            while(rs.next()){
                t2.setText(rs.getString("Name"));
                t3.setText(rs.getString("Last Name"));
            }
        }catch(Exception e){}
	...
	comboBox_3.setModel(new DefaultComboBoxModel(
			new String[] { "B.Tech", "M.Tech", "Weather Sciences", "Environmental Studies", "M.Sc", "B.Sc", "PHD" }));
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource() == b1){
                try{
                conn con = new conn();
                String sql = "insert into fee(ID, Name, Last Name, Course, Branch, semester, fee_paid) values(?, ?, ?, ?, ?, ?, ?)";
